...
# 在主函数开始处添加
device = 'npu'  # 或 'cuda' 或 'cpu'

def create_dispersion_kernel(dm_value, freq_array, time_reso=1e-6, down_time_rate=1):
    """
    为特定DM值创建色散卷积核
    ...
    kernel_size = max(max_delay + 1, 3)  # 至少3个点
...
def npu_dedispersion_v3(data, freq_array, dm_range, time_reso=1e-6, down_time_rate=1):
    ...
    for dm in range(dm_range):
        # 为当前DM创建卷积核
        kernel = create_dispersion_kernel(dm, freq_array, time_reso, down_time_rate)
